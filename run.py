...
def BMRM_collate_fn(batch):
    ...
    # Emotion_inputid = [item['Emotion_inputid'] for item in batch]
    # Emotion_mask=[item['Emotion_mask'] for item in batch]

    # Theme_inputid = [item['Theme_inputid'] for item in batch]
    # Theme_mask=[item['Theme_mask'] for item in batch]

    # Sensitivity_inputid=[item['Sensitivity_inputid'] for item in batch]
    # Sensitivity_mask=[item['Sensitivity_mask'] for item in batch]

    # Rumor_inputid=[item['Rumor_inputid'] for item in batch]
    # Rumor_mask=[item['Rumor_mask'] for item in batch]

    # Match_inputid=[item['Match_inputid'] for item in batch]
    # Match_mask=[item['Match_mask'] for item in batch]

    # embs = [item['embs'] for item in batch]

    label = [item['label'] for item in batch]
    multilabel=[item['multilabel'] for item in batch]

    return {
        'label': torch.stack(label),
        'multilabel':torch.stack(multilabel),
        # 'embs' : torch.stack(embs),
        # 'Emotion_inputid': torch.stack(Emotion_inputid),
        # 'Emotion_mask': torch.stack(Emotion_mask),
        # 'Theme_inputid': torch.stack(Theme_inputid),
        # 'Theme_mask':torch.stack(Theme_mask),
        # 'Sensitivity_inputid':torch.stack(Sensitivity_inputid),
        # 'Sensitivity_mask':torch.stack(Sensitivity_mask),
        # 'Rumor_inputid':torch.stack(Rumor_inputid),
        # 'Rumor_mask':torch.stack(Rumor_mask),
...
class Run():
    ...
    def get_dataloader(self,data_type,data_fold):
        ...
        if data_type=='BMRM':
            # dataset_train = BMRMDataset(f'train{data_fold}.txt')
            # dataset_test = BMRMDataset(f'test{data_fold}.txt')


            collate_fn=BMRM_collate_fn
    ...
    def get_dataloader_temporal(self, data_type):
        collate_fn=None
        if data_type=='BMRM':
            dataset_train = BMRMDataset('vid_time3_train.txt')
            dataset_val = BMRMDataset('vid_time3_val.txt')
            dataset_test = BMRMDataset('vid_time3_test.txt')
            collate_fn=BMRM_collate_fn
        elif data_type=='FANVM':
    ...
    def get_model(self):
        if self.model_name == 'BMRM':
            self.model = BMRMModel(bert_model='/data/code/bert_base_chinese/', fea_dim=128,dropout=self.dropout)
    ...
    def main(self):
        ...
        #     print ("Not Available")
                ...
                #四分类
                history['Acc_all'].append(result['Acc_all'])
                history['F1'].append(result['F1'])
                history['F2'].append(result['F2'])
                history['F3'].append(result['F3'])
                history['F4'].append(result['F4'])

                
            print ('results on 5-fold cross-validation: ')
            for metric in ['Acc_all', 'F1', 'F2', 'F3', 'F4']:
                print ('%s : %.4f +/- %.4f' % (metric, np.mean(history[metric]), np.std(history[metric])))
